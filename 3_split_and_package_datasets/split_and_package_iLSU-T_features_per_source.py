...
i3d_features_list.sort()
complete_i3d_features_list = i3d_features_list
i3d_features_list = []

for source in ['1','2','3']:

	for videoclip in complete_i3d_features_list:
		if videoclip.startswith(source + '__'):
			i3d_features_list.append(videoclip)
	...
	for split in ['test','dev','train']:
		...
		for i in range(a, b):
			...
			if i3dfeats == []:
				print('features vacias')
			if i3dfeats != []:
				sample['sign'] = n1Dtensors_to_2Dtensor(i3dfeats)
				dataset.append(sample)

		# for ASL-2k visual features
		with gzip.open('../packaged/per_source/ilsut.source' + source + '.i3dfeats_2000.span8_stride2.' + split, 'wb') as f:
			pickle.dump(dataset,f)	

		# for BSL-5k visual features
		# with gzip.open('../packaged/per_source/ilsut.source' + source + '.i3dfeats_5383.span8_stride2.' + split, 'wb') as f:
		# 	pickle.dump(dataset,f)	
